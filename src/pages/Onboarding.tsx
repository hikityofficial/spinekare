...
export default function Onboarding() {
    ...
    const handleSelect = (questionId: string, option: string) => {
        ...
        if (step < questions.length - 1) {
            ...
            // Advance to analysis screen 
            setTimeout(() => calculateRisk(newAnswers), 300);
        }
    };

    const calculateRisk = (finalAnswers: Record<string, string>) => {
        ...
        setTimeout(async () => {
            try {
                ...
                // Age Group
                if (finalAnswers['ageGroup'] === '35–50') score += 10;
                if (finalAnswers['ageGroup'] === '50–65') score += 15;
                if (finalAnswers['ageGroup'] === '65+') score += 20;

                // Occupation
                if (finalAnswers['occupationType'] === 'Desk job (8+ hrs sitting)') score += 25;
                if (finalAnswers['occupationType'] === 'Driver') score += 20;
                if (finalAnswers['occupationType'] === 'Physical labor') score += 15;

                // Weightlifting
                if (finalAnswers['isWeightlifter'] === 'Gym' || finalAnswers['isWeightlifter'] === 'Both') score += 10;
                if (finalAnswers['isWeightlifter'] === 'Construction') score += 15;

                // Exercise Frequency (Mitigator)
                if (finalAnswers['exerciseFrequency'] === 'Never') score += 20;
                if (finalAnswers['exerciseFrequency'] === '1–2x week') score += 10;
                if (finalAnswers['exerciseFrequency'] === 'Daily') score -= 10;

                // Pain Level
                if (finalAnswers['painLevel'] === 'Occasionally') score += 15;
                if (finalAnswers['painLevel'] === 'Frequently') score += 25;
                if (finalAnswers['painLevel'] === 'Chronic') score += 35;

                // Posture
                if (finalAnswers['postureAwareness'] === 'I slouch constantly') score += 15;
                if (finalAnswers['postureAwareness'] === 'I am conscious of it') score -= 5;

                // Sleep
                if (finalAnswers['sleepPosition'] === 'Stomach') score += 10;
                ...
                setStep(questions.length); // guarantees progress to results screen
            }
        }, 1500);
    };
